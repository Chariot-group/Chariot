...
let UserService = UserService_1 = class UserService {
    ...
    async create(createUserDto) {
        try {
            ...
            const campaignCheckPromises = campaigns.map((campaignId) => this.campaignModel.findById(campaignId).exec());
            ...
            if (invalidCampaigns.length > 0) {
                const invalidCampaignIds = campaigns.filter((_, index) => !campaignCheckResults[index]);
                const message = `Invalid campaign IDs: ${invalidCampaignIds.join(', ')}`;
                this.logger.error(message, null, this.SERVICE_NAME);
                throw new common_1.BadRequestException(message);
            }
            const goneCampaigns = campaignCheckResults.filter((campaign) => campaign.deletedAt !== null);
            if (goneCampaigns.length > 0) {
                const goneCampaignIds = campaigns.filter((_, index) => campaignCheckResults[index].deletedAt !== null);
