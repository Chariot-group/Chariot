"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var CampaignController_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.CampaignController = void 0;
const common_1 = require("@nestjs/common");
const campaign_service_1 = require("./campaign.service");
const create_campaign_dto_1 = require("./dto/create-campaign.dto");
const update_campaign_dto_1 = require("./dto/update-campaign.dto");
const parse_nullable_int_pipe_1 = require("../../common/pipes/parse-nullable-int.pipe");
const group_service_1 = require("../group/group.service");
const is_creator_guard_1 = require("../../common/guards/is-creator.guard");
const is_creator_decorator_1 = require("../../common/decorators/is-creator.decorator");
let CampaignController = CampaignController_1 = class CampaignController {
    constructor(campaignService, groupService) {
        this.campaignService = campaignService;
        this.groupService = groupService;
        this.SERVICE_NAME = CampaignController_1.name;
        this.logger = new common_1.Logger(this.SERVICE_NAME);
    }
    create(request, createCampaignDto) {
        const userId = request.user.userId;
        return this.campaignService.create(createCampaignDto, userId);
    }
    findAll(request, page, offset, sort, label) {
        const userId = request.user.userId;
        return this.campaignService.findAllByUser(userId, {
            page,
            offset,
            sort,
            label,
        });
    }
    async findAllGroups(request, id, page, offset, sort, label, type = 'all', onlyWithMembers) {
        const userId = request.user.userId;
        let checkCampaginId = await this.campaignService.findOne(id);
        if (checkCampaginId.data) {
            return this.groupService.findAllByUser(userId, { page, offset, sort, label, onlyWithMembers }, id, type);
        }
    }
    findOne(id) {
        return this.campaignService.findOne(id);
    }
    update(id, updateCampaignDto) {
        return this.campaignService.update(id, updateCampaignDto);
    }
    remove(id) {
        return this.campaignService.remove(id);
    }
};
exports.CampaignController = CampaignController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, create_campaign_dto_1.CreateCampaignDto]),
    __metadata("design:returntype", void 0)
], CampaignController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Query)('page', parse_nullable_int_pipe_1.ParseNullableIntPipe)),
    __param(2, (0, common_1.Query)('offset', parse_nullable_int_pipe_1.ParseNullableIntPipe)),
    __param(3, (0, common_1.Query)('sort')),
    __param(4, (0, common_1.Query)('label')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number, Number, String, String]),
    __metadata("design:returntype", void 0)
], CampaignController.prototype, "findAll", null);
__decorate([
    (0, is_creator_decorator_1.IsCreator)(campaign_service_1.CampaignService),
    (0, common_1.Get)(':id/groups'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Query)('page', parse_nullable_int_pipe_1.ParseNullableIntPipe)),
    __param(3, (0, common_1.Query)('offset', parse_nullable_int_pipe_1.ParseNullableIntPipe)),
    __param(4, (0, common_1.Query)('sort')),
    __param(5, (0, common_1.Query)('label')),
    __param(6, (0, common_1.Query)('type')),
    __param(7, (0, common_1.Query)('onlyWithMembers')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, Number, Number, String, String, String, Boolean]),
    __metadata("design:returntype", Promise)
], CampaignController.prototype, "findAllGroups", null);
__decorate([
    (0, is_creator_decorator_1.IsCreator)(campaign_service_1.CampaignService),
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], CampaignController.prototype, "findOne", null);
__decorate([
    (0, is_creator_decorator_1.IsCreator)(campaign_service_1.CampaignService),
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_campaign_dto_1.UpdateCampaignDto]),
    __metadata("design:returntype", void 0)
], CampaignController.prototype, "update", null);
__decorate([
    (0, is_creator_decorator_1.IsCreator)(campaign_service_1.CampaignService),
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], CampaignController.prototype, "remove", null);
exports.CampaignController = CampaignController = CampaignController_1 = __decorate([
    (0, common_1.UseGuards)(is_creator_guard_1.IsCreatorGuard),
    (0, common_1.Controller)('campaigns'),
    __metadata("design:paramtypes", [campaign_service_1.CampaignService,
        group_service_1.GroupService])
], CampaignController);
//# sourceMappingURL=campaign.controller.js.map