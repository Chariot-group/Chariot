    ...
    async seed(clean: boolean) {
        ...
        for (let i = 0; i < userCount; i++) {
            ...
            for (let j = 0; j < campaignsPerUser; j++) {
                ...
                const groupsMainPerCampaign: number = faker.number.int({min: 0, max: 6});
                const groupsPnjPerCampaign: number = faker.number.int({min: 0, max: 6});
                const groupsArchivedPerCampaign: number = faker.number.int({min: 0, max: 6});
