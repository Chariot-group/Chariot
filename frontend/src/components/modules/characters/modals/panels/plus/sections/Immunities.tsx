                    ...
                    { immunities.length > 0 && <ul className="list-disc">
                        {immunities.map((immunitie, index) => (
                            <li key={index} className="text-sm flex flex-row gap-2 items-center">                
                                ...
                                <Input readOnly={!isUpdate} id={index.toString()} type={"text"} value={immunitie ?? ""} onChange={(e) => updateImmunitie(index, e.target.value)} placeholder={t('immunities.name')} className={`w-[10vh] p-0 h-7 border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0` } />
                                {isUpdate && <Tooltip>
                                    <TooltipTrigger asChild>
