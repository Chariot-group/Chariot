...
export default function Abilities({ player }: Props) {

    const [abilities, setAbilities] = useState(player.abilities);

    const addAbility = () => {
        setAbilities([{ name: "", description: "" }, ...abilities]);
    }
                ...
                {abilities.length > 0 && abilities.map((ability, index) => (
                    <Card key={index} className="bg-card p-4 flex justify-between flex-col gap-2 bg-background">
                        <div className="flex flex-row items-center gap-2 w-full">
                            <Champs width="w-full" color="card" label="Nom" value={ability.name} id={`class-${index}`} type={"text"} placeholder={"Classe"} setValue={(value) => updateNameAbility(index, value)} />
                            <Tooltip>
                                <TooltipTrigger asChild>
                        ...
                        <div className="flex flex-col gap-2 w-full h-full">
                            <div className="flex flex-col w-full gap-1.5 h-1/3">
                                ...
                                <Textarea id={"description"} placeholder="Description physique" value={ability.description} onChange={(e) => updateDescriptionAbility(index, e.target.value)} className="h-full rounded-xl resize-none bg-card border-ring" />
                            </div>
