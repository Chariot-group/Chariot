                    ...
                    <Card key={index} className="bg-card p-4 flex justify-between flex-col gap-2 bg-background">
                        ...
                        <div className="flex flex-col gap-2 w-full h-full">
                            <div className="flex flex-col w-full gap-1.5 h-1/3">
                                <Label htmlFor={"description"} className="text-foreground font-bold">{t('abilities.description')}:</Label>
                                <Textarea readOnly={!isUpdate} id={"description"} placeholder={t('abilities.description')} value={ability.description} onChange={(e) => updateDescriptionAbility(index, e.target.value)} className="h-full rounded-xl resize-none bg-card border-ring" />
                            </div>
                        </div>
                    </Card>
                ))}
