                    ...
                    <ul className="list-disc">
                        {senses.map((sense, index) => (
                            <li key={index} className="text-sm flex flex-row gap-2 items-center">                
                                ...
                                <Input readOnly={!isUpdate} id={index.toString()} type={"text"} value={Object.keys(sense) ?? ""} onChange={(e) => updateName(index, e.target.value)} placeholder={t('senses.sensName')} className={`w-[10vh] p-0 h-7 border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0` } />
                                <span> :</span>
                                <Input readOnly={!isUpdate} min={0} id={index.toString()} type={"number"} value={Object.values(sense)[0] ?? 0} onChange={(e) => updateValue(index, e.target.value)} placeholder={t('senses.value')} className={`w-10 p-0 h-7 border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0` } />
                                {isUpdate && <Tooltip>
                                    <TooltipTrigger asChild>
