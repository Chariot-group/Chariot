      ...
      <div className="flex flex-row gap-3">
        <div className="flex flex-col gap-3 w-2/3">
          ...
          <div className="flex flex-row gap-3">
            <MovingCards
              speed={stats.speed}
              setSpeed={(speed) => setStats({ ...stats, speed })}
              isUpdating={isUpdating}
            />
          </div>
        </div>
        <SkillsCard
          skills={stats.skills}
          setSkills={(skills) => setStats({ ...stats, skills })}
          isUpdating={isUpdating}
        />
      </div>
      <div className="flex flex-row gap-3">
        <SensesCard
          senses={stats.senses}
          setSenses={(senses) => setStats({ ...stats, senses })}
          isUpdating={isUpdating}
        />
        <ScoresCard
          abilityScores={stats.abilityScores}
          setAbilityScores={(abilityScores) => setStats({ ...stats, abilityScores })}
          savingThrows={stats.savingThrows}
          setSavingThrows={(savingThrows) => setStats({ ...stats, savingThrows })}
          isUpdating={isUpdating}
        />
      </div>
    </div>
