    ...
    <div className="flex flex-row h-full gap-3 w-full">
      <Card className="flex flex-col gap-2 h-full bg-background p-3">
        ...
        <div className="flex flex-col h-full gap-2">
          {abilities.length === 0 && (
            <div className="text-muted-foreground h-full text-sm flex flex-col justify-center items-center">
              {t("categories.traits.noAbilities")}
            </div>
          )}
          {abilities.length > 0 &&
            abilities.map((ability, index) => (
              <Card
                key={index}
                className={`flex flex-row gap-2 p-3 bg-card items-center justify-between hover:border-2 hover:cursor-pointer ${selectedAbility.name == ability.name ? "border-2" : ""}`}
                onClick={() => {
          ...
          <Card className="flex flex-col gap-2 h-full w-full bg-background p-3">
            ...
            <div className="w-full h-full flex flex-col gap-2">
              <Champs
                ...
                id={"name"}
                type={"text"}
                label={t("labels.traits.abilities.name")}
                onChange={() => {}}
                placeholder={t("placeholders.traits.abilities.name")}
              ...
              <div className="grid w-full max-w-sm items-center gap-1.5">
                <Label
                  ...
                  {t("labels.traits.abilities.description")}
                </Label>
                <Textarea
                  className="h-[10dvh] bg-card"
                  placeholder={t("placeholders.traits.abilities.description")}
